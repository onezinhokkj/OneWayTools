...
class TempMailGenerator:
    ...
    async def run(self, num_emails):
        ...
        if self.email_addresses:
            print(self.text_styler.success("Emails Gerados:") + "\n")
            for email in self.email_addresses:
                print(self.text_styler.success(email))
            while True:
                option = input(self.text_styler.ask("Envie o email gerado para ver as mensagens (ou 'exit' para sair): "))
                if option.lower() == "exit":
                    break
                elif option in self.email_addresses:
                    messages = await self.check_mail(option)
                    if messages:
                        print(self.text_styler.success(f"Mensagens para o email {option}:"))
                        for message in messages:
                            print(message)
                    else:
                        print(self.text_styler.warning(f"Sem mensagens para o email {option}"))
                else:
...
if __name__ == "__main__":
    ...
    num_emails = int(input(styler.ask("Envie o número de emails temporários que deseja criar: ")))
